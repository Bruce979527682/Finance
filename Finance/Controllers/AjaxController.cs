...
namespace Finance.Controllers
{
    public class AjaxController : BaseController
    {
        ...
        public AjaxController(FinanceContext context)
        {
            _context = context;
        }
        ...
        public JsonResult GetTypeList()
        {
            ...
            try
            {
                var list = _context.RecordTypes.AsNoTracking().Where(x => x.UserId.Equals(user.Id)).ToList();
                if (list.Count > 0)
                {
        ...
        public JsonResult GetRecordList()
        {
            ...
            {
                var user = GetUserInfoSession();
                var list = _context.Records.AsNoTracking().Where(x => x.UserId == user.Id && x.AddTime.ToString("yyyy-MM-dd") == date).ToList();
                if (list.Count > 0)
                {
                    return Json(new { code = 1, list });
                }
